...
def check_import_status_view(request, task_id):
    ...
    html_direto = f"""
    <div class="absolute inset-0 z-50 flex items-center justify-center w-screen h-screen p-4 bg-[rgba(0,0,0,0.5)] backdrop-blur transition-opacity flex items-center justify-center">
        <div class="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl transition-all text-center" 
            hx-get="{url_checagem}" 
            hx-trigger="every 2s" 
            hx-swap="outerHTML">
